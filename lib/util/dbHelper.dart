...
class DbHelper {
  ...
  DbHelper._internal(); //internal constructor
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        "CREATE TABLE $tableTodo($colId INTEGER PRIMARY KEY, $colTitle TEXT, " +
  ...
  Future<int> insertTodo(Todo todo) async {
    ...
    return result; // if 0 = error or id
  }
  ...
  Future<int> deleteTodo(int id) async {
    int result;
    var db = await this.db;
    result = await db.rawDelete("DELETE FROM $tableTodo WHERE $colId = $id");
    return result;
  }
}
